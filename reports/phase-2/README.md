# Phase 2 — Single-Site PPO (paper-aligned)

**Status:** PPO converges to a non-trivial, energy-aware policy that
beats always-DPDK by 39% in total reward and 15% in raw energy on
cluster 0 — with **zero QoS violations**. Reward formulation and
observation are aligned with COMCOM-S-26-00430 (Section 4.5 and 4.3),
adapted to a binary action space (DPDK vs USR; horizontal scaling is
a future extension) and to a soft cooldown penalty.
...
Training: 200 000 timesteps, PPO with paper hyperparameters
(`n_steps = 1024`, `learning_rate = 1e-4`, `ent_coef = 0.15`,
`γ = 0.995`, `gae_λ = 0.9`). ~12 min wall on this box.
...
|---|---|---|---|---|---|---|
| **PPO (best checkpoint)** | **−803.61** | **175.30** | **0.00778** | **0.0 %** | 25 % | 81 |
| Always DPDK | −1321.36 | 206.76 | 0.01310 | 0.0 % | 0 % | 0 |
| Threshold (USR < 0.05 Gbps) | −1688.38 | 194.69 | 0.00839 | 2.9 % | — | — |
| Random | −4928.62 | 219.09 | 0.01133 | 16.4 % | 51 % | 511 |
| Always USR | −5640.57 | 230.77 | 0.01067 | 21.7 % | 100 % | 0 |

PPO is the only policy that simultaneously uses USR meaningfully (25 %
of steps) **and** holds the unsafe rate at zero. The threshold rule
loses because once the chosen cutoff is too aggressive the QoS
penalty dominates; the paper's published rule operates with a
dead-band + cooldown guard inside a 3-class configuration space that
isn't a fair point for a binary action without those guards. For this
phase the meaningful comparison is PPO vs always-DPDK.

## Cluster 0 traffic profile

![Cluster 0 load](figures/fig1_load_profile.png)

Bursty around mean 0.108 Gbps, p95 0.225 Gbps, peaks to 0.59 Gbps. The
low-load regime (below ~0.10 Gbps) covers ~30 % of the episode — that
is where PPO's USR usage concentrates.
...
Train from scratch (~12 min wall):
...
- **Late-stage policy collapse exists.** The final-step model scored
  −1078.10 with 1.8 % unsafe; the best-model checkpoint scored
  −803.61 with 0 % unsafe. `EvalCallback` selects the best
  deployment automatically, but the underlying training stability
  deserves a longer investigation later.

The pipeline is now in the shape the paper describes. Phase 3 will
extend the env to multiple clusters in parallel; the reward
formulation, observation construction, and dashboard layering are all
