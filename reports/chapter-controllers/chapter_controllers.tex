...
% TODO(author): the paired bootstrap (p-value) and the phase-2/3/6/7 PNG
% figures were produced under the pre-v0.4 twin; regenerate for the final
% (deltas are within seed noise). The bridge figure IS v0.4-correct.
% ============================================================
...
\midrule
Single-site \ppo{} & $\mathbf{-892.1}$ & $\mathbf{174.3}$ & $0.50$ \\
Always \dpdk{} & $-1321.4$ & $206.8$ & $0.00$ \\
Threshold ($<0.05$\,Gbps) & $-1688.4$ & $194.7$ & $2.87$ \\
\bottomrule
\end{tabular}
\end{center}

The \ppo{} controller improves total reward by $32.5\%$ and energy by $15.7\%$
over always-\dpdk{}, and beats the derived threshold on every axis at once---it
is both safer (0.50\% vs 2.87\% unsafe) and cheaper. The reason is observability:
...
\begin{figure}[t]
  ...
  energy and total penalty. (Figure produced under the pre-\code{v0.4} twin;
  qualitative behaviour is unchanged---regenerate for the final.)}
...
In this work the centralised controller collapsed---on the busiest cluster it
degenerated to always-\usr{}, driving that cluster's unsafe rate above $20\%$,
and its across-seed reward variance was an order of magnitude larger than any
other controller (Figure~\ref{fig:ctrl_collapse}). The opposite design, an
...
\begin{figure}[t]
  ...
  period (green = \dpdk{}, red = \usr{}). The solid red band is the busiest
  cluster collapsed to always-\usr{}: the monolithic joint policy could not
  factor the per-site substructure at this training budget.}
...
\begin{figure}[t]
  ...
  deviation. \mappo{} is the best controller and the most stable trained one.
  (Pre-\code{v0.4} figure; the \code{v0.4} means in Table~\ref{tab:ctrl_headline}
...
paired bootstrap confirms the ordering is significant
(\mappo{}${}>{}$\ippo{}, $p<0.01$).\footnote{The paired bootstrap was last
computed under the pre-\code{v0.4} twin (mean $\Delta=157$, $95\%$ CI
$[39,271]$, $p=0.004$); the $\approx 141$-unit \code{v0.4} gap sits well inside
that interval, so significance is preserved. Regenerate for the final.}
